...
# from .layer import *
# from TiCNet.config import net_config as config
# import numpy as np
# import copy
# import torch
...
#     def __init__(self, config, in_channels=128):
#         super(RpnHead, self).__init__()
#         self.config = config  # ← stocke config comme attribut
#         self.drop = nn.Dropout3d(p=0.5, inplace=False)
#         self.conv = nn.Sequential(
#             nn.Conv3d(in_channels, 64, kernel_size=1),
#             nn.ReLU()
...
#         logits = logits.view(logits.size(0), logits.size(1), -1)
#         logits = logits.transpose(1, 2).contiguous().view(
#             size[0], size[2], size[3], size[4], len(self.config['anchors']), 1)  # ← self.config

#         size = deltas.size()
#         deltas = deltas.view(deltas.size(0), deltas.size(1), -1)
#         deltas = deltas.transpose(1, 2).contiguous().view(
#             size[0], size[2], size[3], size[4], len(self.config['anchors']), 6)  # ← self.config
...
#             torch.zeros(1).cpu(), torch.zeros(1).cpu(), torch.zeros(1).cpu()
...
class RpnHead(nn.Module):
    ...
    def forward(self, f):
        ...
        logits = logits.view(logits.size(0), logits.size(1), -1)
        logits = logits.transpose(1, 2).contiguous().view(
            size[0], size[2], size[3], size[4], len(self.config['anchors']), 1)

        size = deltas.size()
        deltas = deltas.view(deltas.size(0), deltas.size(1), -1)
        deltas = deltas.transpose(1, 2).contiguous().view(
            size[0], size[2], size[3], size[4], len(self.config['anchors']), 6)
...
class CropRoi(nn.Module):
    ...
    def forward(self, f, inputs, proposals):
        ...
        for p in proposals:
            ...
            minimum = torch.LongTensor([[0, 0, 0]])
            maximum = torch.LongTensor(
                (np.array([[self.DEPTH, self.HEIGHT, self.WIDTH]]) / self.scale).astype(int))
            ...
            if np.any((c1 - c0).data.numpy() < 1):
                print(p)
                print('c0:', c0, ', c1:', c1)
                continue  # ← évite le crash si slice vide
...
class MainNet(nn.Module):
    def __init__(self, config, mode='train'):
        ...
        # ← initialise pour éviter AttributeError
        self.rcnn_logits = torch.zeros((0,))
        self.rcnn_deltas = torch.zeros((0,))

    def forward(self, inputs, truth_boxes, truth_labels):
        features, feat_4 = self.feature_net(inputs)  # ← CPU, pas data_parallel
        fs = features[-1]
        ...
        feature_size = fs.shape[2:]
        if self.feature_size is None or self.feature_size != feature_size:
            self.rpn_window = make_rpn_windows(fs, self.cfg)
            self.feature_size = feature_size
        ...
        if self.mode in ['train', 'valid']:
            self.rpn_labels, self.rpn_label_assigns, self.rpn_label_weights, \
                ...
                make_rpn_target(self.cfg, self.mode, inputs, self.rpn_window,
        ...
        if self.use_rcnn:
            if len(self.rpn_proposals) > 0:
                rcnn_crops = self.rcnn_crop(feat_4, inputs, self.rpn_proposals)
                self.rcnn_logits, self.rcnn_deltas = self.rcnn_head(rcnn_crops)  # ← CPU
                self.detections, self.keeps = rcnn_nms(self.cfg, self.mode, inputs,
                                                       self.rpn_proposals,
                                                       self.rcnn_logits, self.rcnn_deltas)
                if self.mode in ['eval']:
                    # ← maintenant DANS le if len > 0
                    fpr_res = get_probability(self.cfg, self.mode, inputs,
                                              self.rpn_proposals,
                                              self.rcnn_logits, self.rcnn_deltas)
                    self.ensemble_proposals[:, 1] = (
                        self.ensemble_proposals[:, 1] * 0.5 + fpr_res[:, 0] * 0.5
                    )
            else:
                # ← aucun candidat RPN, initialise proprement
                self.rcnn_logits = torch.zeros((0,))
                self.rcnn_deltas = torch.zeros((0,))
