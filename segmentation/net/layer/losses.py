...
def focal_tversky_loss(pred, target, alpha=0.7, beta=0.3, gamma=0.75):
    """
    alpha=0.7 : pénalise les faux négatifs → améliore le recall (votre problème)
    beta=0.3  : pénalise les faux positifs
    gamma=0.75: focus sur les cas difficiles
    pred   : logits (N, 1, D, H, W)
    target : float mask (N, 1, D, H, W)
    """
    pred_sig = torch.sigmoid(pred)
    pred_f   = pred_sig.view(pred_sig.size(0), -1)
...
def seg_loss(pred, target):
    return focal_tversky_loss(pred, target)