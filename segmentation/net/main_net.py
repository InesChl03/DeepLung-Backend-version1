...
class MainNet(nn.Module):
    def __init__(self, config, mode='train'):
        ...
        # Tête de segmentation multi-task
        from net.seg_head import SegHead
    ...
    def forward(self, inputs, truth_boxes, truth_labels):
        ...
        if self.use_rcnn:
            if len(self.rpn_proposals) > 0:
                ...
                # Segmentation multi-task sur les mêmes crops
                if self.use_seg and self.mode == 'train':
                    self.seg_logits = self.seg_head(rcnn_crops)
                else:
    ...
    def loss(self):
        ...
        self.seg_loss_val = 0.0
        if self.use_rcnn and hasattr(self, 'seg_logits') \
                and self.seg_logits is not None \
                and hasattr(self, 'seg_targets') \
                and self.seg_targets is not None:
            sl = seg_loss(self.seg_logits, self.seg_targets)
            self.total_loss = self.total_loss + 0.5 * sl
            self.seg_loss_val = sl.item()
