...
from ticnet.net.layer.losses import *
...
def rcnn_loss(logits, deltas, labels, targets):
    ...
    if num_pos > 0:
        # one hot encode
        select = Variable(torch.zeros((batch_size, num_class)))
    ...
    for slice_idx in range(len(truth_box)):
        ...
        for nodule_rpn in slice_rpn: 
            ...
            for nodule_box in truth_box[slice_idx]:
                if box_ciou(nodule_rpn, torch.from_numpy(nodule_box)) < 0:
                    print("Unavailable box.")
                iou_score = box_ciou(nodule_rpn, torch.from_numpy(nodule_box))
                iou_loss += iou_score
