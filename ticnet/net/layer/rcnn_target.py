import copy
from ticnet.net.layer.rcnn_nms import rcnn_encode
import time
import random
import numpy as np
import torch
...
try:
    from ticnet.utils.pybox import *
except ImportError:
    print('Warning: C++ module import failed! This should only happen in deployment')
    from ticnet.utils.util import py_nms as torch_nms
    from ticnet.utils.util import py_box_overlap as torch_overlap
...
def make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label):
    sampled_proposal = torch.zeros((0, 8)).float()
    sampled_label = torch.zeros((0, 1)).long()
    sampled_assign = np.zeros((0, 1), dtype=np.int32) - 1
    sampled_target = torch.zeros((0, 6)).float()
    ...
    if len(truth_box) == 0:
        ...
        sampled_label = torch.zeros((num_bg)).long()
    ...
    sampled_target   = Variable(torch.from_numpy(sampled_target)).float()
