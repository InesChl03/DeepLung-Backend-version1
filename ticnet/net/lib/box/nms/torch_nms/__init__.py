from ticnet.net.lib.box.nms.torch_nms.extension import *
...
def torch_nms(dets, thresh):
    ...
    if not dets.is_cuda:
        ...
        areas = d ** 3
        order = scores.sort(0, descending=True)[1]
        # order = torch.from_numpy(np.ascontiguousarray(scores.cpu().numpy().argsort()[::-1])).long()
        ...
        # return dets[keep[:num_out[0]]].contiguous(), keep[:num_out[0]]
        return dets[keep[:num_out[0]]].contiguous(), order[keep[:num_out[0]]].contiguous()
