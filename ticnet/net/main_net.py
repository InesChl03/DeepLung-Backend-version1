...
from .layer import *
from ticnet.ticnet_config import net_config as config
...
class RcnnHead(nn.Module):
    def __init__(self, cfg, in_channels=128):
        ...
        self.fc1 = nn.Linear(in_channels * self.crop_size[0] * 
                             self.crop_size[1] * self.crop_size[2], 512)
        self.fc2 = nn.Linear(512, 256)
        self.logit = nn.Linear(256, self.num_class)
        self.delta = nn.Linear(256, self.num_class * 6)

        # AJOUTER SEULEMENT CES 2 LIGNES
    ...
    def forward(self, crops):
        x = crops.view(crops.size(0), -1)
        x = F.relu(self.fc1(x), inplace=True)
        x = self.drop1(x)                      # AJOUTER
        x = F.relu(self.fc2(x), inplace=True)
        x = self.drop2(x)                      # AJOUTER
        logits = self.logit(x)
        deltas = self.delta(x)
        return logits, deltas
...
#     self.rcnn_logits = torch.zeros(1)
#     self.rcnn_deltas = torch.zeros(1)
...
#             if self.mode in ['eval']:
#                 # Ensemble
#                 fpr_res = get_probability(self.cfg, self.mode, inputs, self.rpn_proposals,
#                                           self.rcnn_logits, self.rcnn_deltas)
#                 self.ensemble_proposals[:, 1] = self.ensemble_proposals[:, 1] * 0.5 + fpr_res[:, 0] * 0.5
